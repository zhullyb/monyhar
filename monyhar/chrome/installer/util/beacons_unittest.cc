...
TEST_P(BeaconTest, Location) {
  ...
  ASSERT_FALSE(
      ...
      << wrong_key;
#else
  // The tests above are skipped for Monyhar builds because they fail for two
  // reasons:
  // - ClientState and ClientStateMedium are both Software\Monyhar.
  // - the registry override manager does its virtualization into
  //   Software\Monyhar, so it always exists.
