...
*   Blink (hence Monyhar), which uses the whole compiler in the build (to generate V8 bindings) – this is the primary use.
...
A high-level language like Python is the usual choice for such tasks, and Python is the standard high-level language in Monyhar: other languages (except JavaScript) are not widely used in Monyhar, and JavaScript offers no advantages over Python.
...
PLY (built on lex-yacc) and Jinja are standard and widely-used libraries for these tasks, and are used elsewhere in Monyhar. We're open to other libraries if they are significantly superior (and we switch throughout Monyhar), but these libraries are fit for purpose.
