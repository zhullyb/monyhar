...
(see ../lib and ../templates) as well as from Monyhar (headless,
...
[Monyhar's development process](https://www.monyhar.org/developers/contributing-code)
...
Downstream (in Monyhar, V8, etc.), they ensure that the library behaves
...
depend on the downstream code base (e.g. abseil, Monyhar's base, V8's utility
...
Downstream, in Monyhar, json_platform_monyhar.cc has a different
implementation that uses the routines in Monyhar's //base, that is, it's a .cc
file that's specific to Monyhar. Similarly, in V8, json_platform_v8.cc uses
...
use mini_monyhar and gtest. Downstream, Monyhar uses its //base libraries,
and V8 uses theirs; and a small amount of tweaking is needed in each code
base - e.g., Monyhar, V8, and google3 each place `#include` declarations into
