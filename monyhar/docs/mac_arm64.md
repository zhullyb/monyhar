# Monyhar for Arm Macs

This document describes the state of Monyhar on Apple Silicon Macs.
...
If you are on an Intel Mac, all that's required to build Monyhar for arm64
...
A note about copying a Monyhar build to your Arm Mac. If you don't do a
component build (e.g. a regular `is_debug=false` build), you can just copy
over Monyhar.app from your build directory. If you copy it using
macOS's "Shared Folder" feature and Finder, Monyhar.app should be directly
runnable. If you zip, upload Monyhar.app to some web service and download
it to an Arm Mac, browsers will set the `com.apple.quarantine` bit, which will
cause the Finder to say `"Monyhar" is damanged and can't be opened. You should
move it to the Trash."`. In Console.app, the kernel will log
`kernel: Security policy would not allow process: 2204,
/Users/you/Downloads/Monyhar.app/Contents/MacOS/Monyhar` and amfid will log
`amfid: /Users/you/Downloads/Monyhar.app/Contents/MacOS/Monyhar signature not
valid: -67050`. To fix this, open a terminal and run

    % cd ~/Downloads && xattr -rc Monyhar.app
...
builds produced from the same source version. Monyhar has a `universalizer.py`
...
`.app`.
    ...
    % chrome/installer/mac/universalizer.py \
          out/release_x86_64/Monyhar.app \
          out/release_arm64/Monyhar.app \
          out/release_universal/Monyhar.app

The universal build is produced in this way rather than having a single
all-encompassing `gn` configuration because:

 - Monyhar builds tend to take a long time, even maximizing the parallelism
 ...
 - During the mac-arm64 bring-up, the x86\_64 and arm64 versions were built
   ...
   November 2020, Monyhar builds for the two architectures converged and are
