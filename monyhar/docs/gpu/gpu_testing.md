...
The GPU bots run a different set of tests than the majority of the Monyhar
...
external scripts written in Python. The GPU bots launch the full Monyhar
...
page in order to validate Monyhar's rendering architecture end-to-end. Where
...
Most Monyhar developers interact with the GPU bots in two ways:
...
The GPU tests are part of the default set for Monyhar CLs, and are run as part
...
this writing, they ran longer-running tests that can't run against all Monyhar
...
interface. They are configured similarly to the tryservers for regular Monyhar
...
Monyhar (`src/`) and ANGLE (`src/third_party/angle/`), please
...
All of the GPU tests running on the bots can be run locally from a Monyhar
...
which aren't allowed to run on the regular Monyhar waterfalls:
...
be replaced with an actual Monyhar src revision (typically whatever revision
...
To create a zip archive of your personal Monyhar build plus all of
...
command failed to zip up a working Monyhar binary. The browser failed
...
and the Monyhar binaries moved over to the target machine. Then the
...
*   GTests: those which use the Googletest and Monyhar's `base/test/launcher/`
    frameworks.
...
Monyhar CL by virtue of the `linux-rel`, `mac-rel`, `win7-rel` and
...
reliable automated test machines in the Monyhar project. Let's keep them that
way.
...
*   Multiple bugs in Monyhar's support for lost contexts causing flakiness of
